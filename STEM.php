...
if(isset($_POST["delete"])){
    ...
    $id = $_POST["ID"];
    $sql = "DELETE FROM shs_thesis WHERE id = '$id'";
    $con->query($sql) or die ($con->error);
    echo header ("Location: Shs_thesis.php");

}     


$sql = "SELECT * FROM shs_thesis WHERE strand = 'SCIENCE TECHNOLOGY, ENGINEERING, AND MATHEMATICS'";
...
<header class="">
      <div class="navbar-expand-lg container-fluid p-2 fixed-top" style="background-color:red;">
        ...
        <img class="mx-3" src="./Images/Burgos logo.png" alt="" style="max-width: 60px;">
            <span class="lead">Senior High Alumni List</span>
        </span>
        ...
        <section class="container-fluid mt-3" id="">
            <br>
            <br>
            <h3 class="text-center my-4">SCIENCE TECHNOLOGY, ENGINEERING, AND MATHEMATICS</h3>

           <a href="Shs_thesis.php" class="btn btn-primary my-2" style="margin-left:94%;">Back</a>
            <?php if($row>0){ ?>
            ...
            <table id="table" class="table table table-hover text-center" style="width: 100%;">
                ...
                <tbody class="table-secondary table-group-divider">
                    <?php foreach($files2 as $file): ?>
                        ...
                        <tr>
                            <td><?php echo $file['strand'];?></td>
                            <td><?php echo $file['name'];?></td>
                            <td>
                                <a href="Shs_thesis.php?file_id=<?php echo $file['id']?>"><div class="btn btn-primary">Download</div></a>
                            </td>
                          
                    <form action="ABM.php" method="post">
                        <input type="hidden" name="ID" value="<?php echo $row["id"];?>">
                        <td><button type="submit" name="delete" class="btn btn-danger px-3" OnClick="return confirm('Are you sure you want to delete this Student?');">Delete</td>
                    </form>
                    </tr>
                    <?php endforeach ; ?>
                </tbody>
                <?php }else{ ?>
                <h1 class="No_data_STEM text-center">NO DATA FOUND!</h1>
                <?php } ?>
                </table>
        </section>
